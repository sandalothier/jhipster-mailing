package com.fisc.mailing.web.rest;

import com.fisc.mailing.config.KafkaProperties;
...
class MailingKafkaResourceIT {
    ...
    void setup() {
        ...
        MailingKafkaResource kafkaResource = new MailingKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/mailing-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/mailing-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
